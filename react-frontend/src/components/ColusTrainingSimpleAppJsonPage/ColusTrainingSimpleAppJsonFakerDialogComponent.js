...
import colusTrainingSimpleAppJsonFakerFactory from "./colusTrainingSimpleAppJsonFakerFactory";
...
const ColusTrainingSimpleAppJsonFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = colusTrainingSimpleAppJsonFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("colusTrainingSimpleAppJson").create(elem));
...
export default connect(null, mapDispatch)(ColusTrainingSimpleAppJsonFakerDialogComponent);
