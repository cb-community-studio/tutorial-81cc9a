...
import seederData from "./colusTrainingSimpleAppJson.seeder.json";
...
const ColusTrainingSimpleAppJsonSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("colusTrainingSimpleAppJson").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(ColusTrainingSimpleAppJsonSeederDialogComponent);
